...
class OSSUtil:
    ...
    def get_default_file_key(self, url, is_thumbnail=False):
        ...
        res = f"tools/{image_name}/screenshot.png"
        if is_thumbnail:
            res = f"tools/{image_name}/screenshot-thumbnail.png"
    ...
    def upload_file_to_r2(self, file_path, file_key):
        try:
            ...
            return None
    def upload_streams_to_r2(self, streams, file_key):
        try:
            self.s3.put_object(Body=streams, Bucket=self.S3_BUCKET_NAME, Key=file_key)

            logger.info(f"文件 '{file_key}' 成功上传到 '{self.S3_BUCKET_NAME}/{file_key}'")
    ...
    def generate_thumbnail_image(self, file_path, file_key):
